...
public class Background extends AsyncTask<String,Void,String> {
    ...
    private String getEmpLog(String mImage, String empLog_url, String mId, String mDate, String mLongitude, String mLatitude) {
        try {
            ...
            String post_data = URLEncoder.encode("emp_id","UTF-8")+"="+URLEncoder.encode(mId,"UTF-8")
                    ...
                    +"&"+URLEncoder.encode("empDate","UTF-8")+"="+URLEncoder.encode(mDate,"UTF-8")+"&"+mImage;
    ...
    private String getLocation(String mlocation, String empLocation_url, String mId, String mLongitude, String mLatitude) {
        try {
            ...
            OutputStream outputStream= httpURLConnection.getOutputStream();
            BufferedWriter bufferedWriter = new BufferedWriter(new OutputStreamWriter(outputStream,"UTF-8"));
            String post_data = URLEncoder.encode("emp_id","UTF-8")+"="+URLEncoder.encode(mId,"UTF-8")+"&"+URLEncoder.encode("mLongitude","UTF-8")
                           +"="+URLEncoder.encode(mLongitude,"UTF-8")+"&"+URLEncoder.encode("mLatitude","UTF-8")
                    +"="+URLEncoder.encode(mLatitude,"UTF-8")+"&"+mlocation;
            bufferedWriter.write(post_data);
            bufferedWriter.flush();
            bufferedWriter.close();
            outputStream.close();
